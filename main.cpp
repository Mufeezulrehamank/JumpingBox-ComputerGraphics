...
// flags declaration
int COLLISIONCHECKTIME = 2; // check for collision ever millisecond
int JUMPSPEED = 5; // speed of jump: 125 pixel/sec, jump time: 2.4sec
int MAX_HEIGHT = 180; // max height that can be reached by T_Rex: 150
int NUM_CACT = 25; // number of cactus position stored: 25
int DASH_CACT_SAND = 115; // speed of cactus: 333.33 pixels/sec, Time to attack: 6 sec
...
int DAYCYCLE = 30; // how fast does the time updates
...
char textb[] = {" Computer Graphics Mini Project By MOHAMMED ALI & MUFEEZUL REHAMAN K"};
char textc[] = {"Department Of Computer Science and Engineering, UVCE "};
...
char textj[] = {" 4) Press <ESC Key> to quit the game"};
...
void drawCoverPage() // drawing the cover page
{
    ...
    drawText(0600, 400, textb); // draw textb
    drawText(0650, 350, textc); // draw textc
    drawText(0300, 300, textd); // draw textd
    drawText(0300, 250, texte); // draw texte
    drawText(0700, 225, textf); // draw textf
    drawText(0700, 200, textg); // draw textg
    drawText(0700, 175, texth); // draw texth
    drawText(0700, 150, textj); // draw textj
}
void drawScnFreez() // displaying the score and GAME OVER
{
    ...
    drawText(800, 300, "BOX is damaged"); // display trex is hurt
    drawText(800, 250, result); // display the result
}
